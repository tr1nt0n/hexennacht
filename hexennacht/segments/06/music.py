...
trinton.append_rests(score=score, voice="percussion 2 voice", rests=[abjad.Rest("r8")])
...
trinton.handwrite(
    score=score, voice="percussion 2 voice", durations=[(7, 8)], pitch_list=None
)
...
handler = evans.PitchHandler(pitch_list=hexennacht.ratchet_pitches, forget=False)
...
hexennacht.string_swells(
    score=score, voice="cello voice", tuplet_index=0, durations=[(1, 1)], pitch_index=0
)
...
hexennacht.string_swells(
    score=score, voice="cello voice", tuplet_index=1, durations=[(3, 4)], pitch_index=3
)
...
hexennacht.string_swells(
    score=score, voice="cello voice", tuplet_index=2, durations=[(3, 4)], pitch_index=6
)

trinton.append_rests(
    score=score, voice="cello voice", rests=[abjad.Rest("r4."), abjad.Rest("r2")]
)

hexennacht.string_swells(
    score=score, voice="cello voice", tuplet_index=3, durations=[(1, 2)], pitch_index=9
)
...
hexennacht.drone(
    score=score, voice="contrabass voice", talea=16, pitch_index=11, durations=[(7, 4)]
)
...
trinton.attach(voice=score["Global Context"], leaves=[0], attachment=hexennacht.tempo_4)
...
trinton.write_trill_span(
    score=score, voice="trumpet voice", pitch=19.5, start_leaf=[6], stop_leaf=[7]
)

trinton.write_trill_span(
    score=score, voice="trumpet voice", pitch=22, start_leaf=[10], stop_leaf=[11]
)

trinton.write_trill_span(
    score=score, voice="trumpet voice", pitch=14.5, start_leaf=[13], stop_leaf=[15]
)
...
trinton.attach(
    ...
    attachment=abjad.LilyPondLiteral(r'\boxed-markup "CLB" 1', format_slot="after"),
)
...
trinton.attach(
    ...
    attachment=abjad.LilyPondLiteral(
        r'\boxed-markup "Bass drum, w/ mallet in RH and palm in LH" 1',
        format_slot="after",
    ),
)
...
trinton.attach_multiple(
    ...
    attachments=[abjad.Dynamic("ff"), abjad.Articulation("tenuto")],
)
...
trinton.change_notehead(voice=score["cello voice"], leaves=all, notehead=r"#'harmonic")
