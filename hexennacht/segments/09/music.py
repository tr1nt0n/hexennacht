...
for voice in [
    ...
    trinton.append_rests(score=score, voice=voice, rests=[abjad.Rest("r1..")])
...
trinton.append_rests(score=score, voice="flute voice", rests=[abjad.Rest("r1")])
...
trinton.handwrite(
    score=score, voice="percussion 2 voice", durations=[(1, 4)], pitch_list=[4]
)

trinton.transparent_accidentals(score=score, voice="percussion 2 voice", leaves=all)
...
for tempo, leaf, in zip(
    [hexennacht.tempo_3, hexennacht.tempo_2, hexennacht.tempo_1],
    [
        1,
        4,
        7,
...
trinton.attach(
    voice=score["Global Context"], leaves=[10], attachment=abjad.BarLine("|.")
)
...
trinton.attach(voice=score["oboe voice"], leaves=[28], attachment=abjad.Dynamic("ff"))
